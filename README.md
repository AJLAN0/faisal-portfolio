...
| **Portfolio works** (title, category, image, **`videoCount`**) | `portfolio.items` |
...
### Works = video collections

Each project in **Selected Works** (`أعمال مختارة`) is a *collection*: the card
shows its clip count and opens a lightbox gallery with one cinematic tile per
clip. The number comes from `portfolio.items[].videoCount`; the per‑clip
placeholder thumbnails live in `public/images/works/<slug>-<n>.svg` and are
regenerated by `npm run gen:placeholders`. To wire **real** videos, give each
clip a URL in `src/components/ui/Lightbox.tsx` and turn the tiles into
links/players.
